...
// 使用 JSON Schema 校验
function checkExtensionsFormat(extensions) {
  return new Promise((resolve, reject) => {
    if (Array.isArray(extensions)) {
      let errorMsg = [];
      extensions.forEach((item) => {});
      if (errorMsg) {
        reject("extensions firstName cannot by empty");
      } else {
      }
    } else {
      reject("extensions must by Array");
    }
  });
}

function checkExtensionsItemFormat({
  firstName = "",
  lastName = "",
  ext = "",
  extType = "",
  ...
  if (firstName === "") {
    errors.push("extensions firstName cannot by empty");
  }

  if (extType !== "") {
    if (
      !["DigitalUser", "VirtualUser", "FaxUser", "Dept", "AO"].includes(extType)
    ) {
      errors.push(
        'extensions extType can only has "DigitalUser", "VirtualUser","FaxUser","Dept","AO"'
      );
    }
  } else {
    errors.push("extensions extType cannot by empty");
  }
...
function sortExtensionsByName(extensions) {
  checkExtensionsFormat(extensions)
    .then(() => {})
    .catch((err) => {
      console.log(err);
    });
